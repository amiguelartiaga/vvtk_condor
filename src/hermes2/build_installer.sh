#!/bin/bash
#
# build_installer.sh — generates the self-extracting condor_hermes2 installer
#
set -euo pipefail
...
#!/bin/bash
# ============================================================================
#  condor_hermes2 — interactive installer for hermes2 tools
#
#  Install:  curl -fsSL <URL>/condor_hermes2 | bash
#        or: bash condor_hermes2
# ============================================================================
{
set -e
...
echo -e "${BOLD}======================================${RESET}"
echo -e "${GREEN}  condor_hermes2 installer${RESET}"
echo -e "${BOLD}======================================${RESET}"
echo ""
...
if echo "$PATH" | tr ':' '\n' | grep -qx "$INSTALL_DIR"; then
	...
	if [[ "$ADD_PATH" =~ ^[Yy] ]]; then
		echo "" >> "$BASHRC"
		echo "# condor_hermes2 tools" >> "$BASHRC"
...
echo ""
echo -e "  Run ${CYAN}condor_hermes2${RESET} or one of the installed scripts to get started."
